...
if (!htmlPath) {
  console.error(
    "Usage: node validate-company-evidence-report.mjs --html <report.html> [--expected-accounts N] [--expected-financials N]",
  );
  process.exit(2);
}
...
const commercialBeforeFinancials =
  ...
  profileBlocks.every((block) => {
    ...
    const financialSection = block.indexOf('class="profile-financials"');
    return commercial !== -1 && financialSection > commercial;
  });
...
const checks = [
  ...
  },
  {
    ...
    actual:
      /--step-0:\s*clamp\(/i.test(html) && /--space-s:\s*clamp\(/i.test(html),
    expected: true,
  },
  {
    name: "explicit missing-value language",
    ok: /Not disclosed|No accounts filed|PDF-only/i.test(html),
    actual: /Not disclosed|No accounts filed|PDF-only/i.test(html),
    expected: true,
  },
  {
    name: "responsive viewport",
    ok: /<meta\b[^>]*name=["']viewport["']/i.test(html),
    actual: /<meta\b[^>]*name=["']viewport["']/i.test(html),
    expected: true,
  },
  {
    name: "print styles",
    ok: /@media\s+print/i.test(html),
    actual: /@media\s+print/i.test(html),
    expected: true,
  },
];
